  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
        <button
          onClick={prev} style={navBtnStyle}
        ...
        >‹</button>
        <div style={{ fontFamily: "'DM Serif Display',serif", fontSize: 15, color: "#E2E8F0" }}>
          {MONTHS[month]} {year}
        </div>
        <button
          onClick={next} style={navBtnStyle}
          onMouseEnter={e => { e.currentTarget.style.color = "#E2E8F0"; e.currentTarget.style.background = "rgba(255,255,255,0.06)"; }}
          onMouseLeave={e => { e.currentTarget.style.color = "#4A5568"; e.currentTarget.style.background = "none"; }}
        >›</button>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 2 }}>
        {/* Day-of-week headers */}
        {DOWS.map(d => (
          <div key={d} style={{ textAlign: "center", fontSize: 9, color: "#2D3748", fontWeight: 700, padding: "4px 0", letterSpacing: "0.06em" }}>
            {d}
          </div>
        ))}
        ...
        {Array.from({ length: daysInMon }, (_, i) => {
          ...
          return (
            <div
              key={d}
              onClick={() => hasTasks && setSelected(isSel ? null : dateStr)}
              style={{
