...
function TaskCard({ task, onCycle, onDelete, onEdit }) {
  ...
  const borderColor = task.status === "In Progress" && task.deadlineDate && task.deadlineDate < today
    ? "rgba(232,69,69,0.2)"
    : task.status === "Not Started" && task.startDate && task.startDate < today
    ? "rgba(245,166,35,0.2)" : "rgba(255,255,255,0.05)";

  return (
    <div onClick={() => onEdit(task.id)} style={{
      background:"linear-gradient(145deg,#0E1826,#0A1220)",
      border:`1px solid ${borderColor}`, borderLeft:`3px solid ${pr.color}`,
    ...
    >
      <div style={{ display:"flex", gap:12, alignItems:"flex-start" }}>
        <div style={{ flex:1, minWidth:0 }}>
          ...
          {task.notes && <div style={{ marginTop:8, color:"#3A4A5C", fontSize:11, borderTop:"1px solid rgba(255,255,255,0.04)", paddingTop:8 }}>
